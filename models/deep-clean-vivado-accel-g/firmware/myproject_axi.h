...
static const unsigned N_OUT = 8192;
typedef float T_in;
typedef float T_out;
typedef struct in_struct {
    ...
    { return stream << "{ data: " << float(in.data) << ", last: " << float(in.last) << " }" << std::endl; }
...
typedef struct out_struct {
    ...
    { return stream << "{ data: " << float(out.data) << ", last: " << float(out.last) << " }" << std::endl; }
    operator float() const {return this->data;}
    operator double() const {return this->data;}
